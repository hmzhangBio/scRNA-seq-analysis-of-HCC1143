...
#load the UMI matrix
```{r load the dataset}
Ctrl_72h.data=read.table(file=path_of_HCC1143_72h_Ctrl_UMI_count.txt, sep="\t", header = TRUE, row.names = 1)
Ctrl_72h=CreateSeuratObject(raw.data = Ctrl_72h.data, project = "Control")
Treated_72h.data=read.table(file=path_of_HCC1143_72h_Treated_UMI_count.txt, sep="\t", header = TRUE, row.names = 1)
Treated_72h=CreateSeuratObject(raw.data = Treated_72h.data, project = "Paclitaxel")
```
#merge two datasets
```{r}
combined_72h <- MergeSeurat(object1 = Ctrl_72h, object2 = Treated_72h, project = "72h")
combined_72h
```
#quality control of datasets
```{r}
# calculate the percent.mito values.
mito.genes <- grep(pattern = "^MT-", x = rownames(x = combined_72h@data), value = TRUE)
percent.mito <- Matrix::colSums(combined_72h@raw.data[mito.genes, ])/Matrix::colSums(combined_72h@raw.data)
combined_72h <- AddMetaData(object =combined_72h, metadata = percent.mito, col.name = "percent.mito")
#visualize QC metrics
VlnPlot(object = combined_72h, features.plot = c("nGene", "nUMI", "percent.mito"), nCol = 3)
#filter low quality cells
combined_72h <- FilterCells(combined_72h, subset.names = c("nUMI", "nGene", "percent.mito"), low.thresholds=c(10000, 2000, -Inf), high.thresholds= c(Inf, Inf, 0.13))
```

##Normalizing data
```{r normalization}
#normalization
combined_72h <- NormalizeData(object = combined_72h, normalization.method = "LogNormalize", scale.factor = 50000)
#scale the data and regress out UMI count
combined_72h=ScaleData(object = combined_72h, vars.to.regress="nUMI", model.use = 'negbinom', display.progress = TRUE)
```
...
```{r}
combined_72h <- CellCycleScoring(object = combined_72h, s.genes = s.genes, g2m.genes = g2m.genes, set.ident = TRUE)
```
#stratify cells by cell cycle phase
```{r}
G1_cells=row.names(combined_72h@meta.data)[combined_72h@meta.data$Phase=='G1']
G1=SubsetData(combined_72h, cells.use=G1_cells)

S_cells=row.names(combined_72h@meta.data)[combined_72h@meta.data$Phase=='S']
S=SubsetData(combined_72h, cells.use=S_cells)

G2M_cells=row.names(combined_72h@meta.data)[combined_72h@meta.data$Phase=='G2M']
G2M=SubsetData(combined_72h, cells.use=G2M_cells)
```
...
G1.raw.data <- combined_72h@raw.data[, G1_cells]
...
G1@meta.data <- combined_72h@meta.data[G1_cells,]
```
...
#Cluster cells
```{r}
G1 <- FindClusters(object=G1, reduction.type = "pca",  dims.use = 1:30, print.output = 0, save.SNN = TRUE, force.recalc=TRUE, resolution=0.3)
```
#Dimplot
```{r}
G1 <- RunTSNE(G1, dims.use = 1:30, do.fast = T, perplexity=38)
p1=DimPlot(object = G1, label = TRUE, reduction.use ='tsne', group.by = "orig.ident", cols.use=c("#00e6e6", "#ff80ff"), no.axes =TRUE)
p2=DimPlot(object = G1, label = TRUE, reduction.use ='tsne', group.by = "res.0.3", cols.use=c("#cca300", "#004080"), no.axes =FALSE)
```
...
S.raw.data <- combined_72h@raw.data[, S_cells]
#filter rarely expressed genes
...
S@meta.data <- combined_72h@meta.data[S_cells,]
```
#Identification of highly variable genes
...
#Cluster cells
```{r}
S <- FindClusters(object=S, reduction.type = "pca",  dims.use = 1:30, print.output = 0, save.SNN = TRUE, force.recalc=TRUE, resolution=0.3)
```
#Dimplot
```{r}
S <- RunTSNE(S, dims.use = 1:30, do.fast = T, perplexity=38)
p1=DimPlot(object = S, label = TRUE, reduction.use ='tsne', group.by = "orig.ident", cols.use=c("#00e6e6", "#ff80ff"), no.axes =FALSE)
p2=DimPlot(object = S, label = TRUE, reduction.use ='tsne', group.by = "res.0.3", cols.use=c("#990073", "#86b300"), no.axes =FALSE)
```
#Finding differentially expressed genes (cluster biomarkers)
```{r}
...
G2M.raw.data <- combined_72h@raw.data[, G2M_cells]
#filter rarely expressed genes
...
G2M@meta.data <- combined_72h@meta.data[G2M_cells,]
```
#Identification of highly variable genes
```{r find variable genes}
...
#Cluster cells
```{r}
G2M <- FindClusters(object=G2M, reduction.type = "pca",  dims.use = 1:30, print.output = 0, save.SNN = TRUE, force.recalc=TRUE, resolution=0.5)
```
#Dimplot
```{r}
...
p2=DimPlot(object = G2M, label = TRUE, reduction.use ='tsne', group.by = "res.0.5", cols.use=c("#996600", "#339966"))
```
#Finding differentially expressed genes (cluster biomarkers)
```{r}
